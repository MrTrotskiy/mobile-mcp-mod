...
describe("iphone-simulator", () => {
	...
	it("should be able to send keys and press enter", async function() {
		...
		// Wait for Reminders app to load and "New Reminder" to appear
		const reminderResult = await simctl.waitFor({
			type: "element_visible",
			element: { label: "New Reminder" },
			timeout: 5000
		});
		assert.ok(reminderResult.success, "New Reminder element should appear");
		const newElement = reminderResult.element;
		...
		await simctl.waitFor({ type: "time", timeMs: 800 });
	...
	it("should be able to open url", async function() {
		hasOneSimulator || this.skip();
		// Check that openurl with https:// launches Safari
	...
	it("should be able to get elements on screen", async function() {
		...
		await simctl.waitFor({ type: "time", timeMs: 1500 });
	...
	it("should be able to launch and terminate app", async function() {
		hasOneSimulator || this.skip();
		await restartPreferencesApp();
		// Wait for Preferences app to load
		await simctl.waitFor({ type: "time", timeMs: 1500 });
		const elements = await simctl.getElementsOnScreen();
